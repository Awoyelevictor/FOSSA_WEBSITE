...
export const DepartmentsPage = () => {
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-16 text-left">
            ...
            <AnimateStaggerContainer delay={0.1} className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {departments.map((dept, idx) => (
                    <AnimateStaggerItem key={idx}>
                        <GlareHover borderRadius="2rem" style={{ height: '100%' }}>
            ...
            {/* Scroll Reveal Quote Block at the bottom */}
            <section className="mt-20 mb-12 px-6 md:px-12 py-16 bg-slate-50 dark:bg-slate-900/40 border border-slate-100 dark:border-slate-800/85 rounded-[2.5rem] text-center shadow-sm">
                <p className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-[0.25em] mb-4">Interdisciplinary Future</p>
                <ScrollReveal
                    baseOpacity={0.15}
                    enableBlur={true}
                    baseRotation={2}
                ...
                >
                    The greatest discoveries of our age do not occur inside isolated disciplines, but at the intersection of computing, biology, chemistry, mathematics, and physical sciences.
                </ScrollReveal>
            </section>
        </div>
    );
