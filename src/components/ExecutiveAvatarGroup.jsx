...
export const ExecutiveAvatarGroup = () => {
    return (
        <AvatarGroup className="h-14 sm:h-24 md:h-32 -space-x-2.5 sm:-space-x-5 md:-space-x-7 justify-center flex-nowrap" translate="-35%">
            {executivesData.map((exec, index) => (
                <Avatar key={index} className="size-12 sm:size-20 md:size-28 shrink-0 border border-white dark:border-slate-800 shadow-xl cursor-pointer hover:scale-105 transition-transform duration-300 relative">
                    <AvatarImage src={exec.avatar} alt={exec.name} className="object-cover" referrerPolicy="no-referrer" />
                    <AvatarFallback className="bg-gradient-to-br from-indigo-500 to-purple-600 text-white font-bold text-xs sm:text-base md:text-lg">
